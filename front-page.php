		...
		for ($i=0; $i < count($testimonials); $i=$i+2) : ?>
			<div>
				<?php 
					$t1 = $testimonials[$i];
					$t2 = $testimonials[$i+1];
					get_template_part( 'template-parts/cards/testimonial','',array('testimonial' => $t1 ) ); 
					if($t2)
					{
						get_template_part( 'template-parts/cards/testimonial','',array('testimonial' => $t2 ) ); 
					}
				?>
			</div>
	<?php	endfor; ?>
