...
<div class="wc-MyAccount-sub-heading">
	<h2 class="title is-3"><?php _e( 'Tus pedidos', 'arkdewp' ); ?></h2>
</div>
...
<?php if ( $has_orders ) : ?>
	...
	<table class="mt-6 woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		<thead>
			<tr>
		...
		<tbody>
			...
			foreach ( $customer_orders->orders as $customer_order ) {
				...
				<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?> pt-3" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button button is-small is-purple is-outlined ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
									}
								}
								?>
							<?php endif; ?>
