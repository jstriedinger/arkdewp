...
$course_cats_string = '';
if ( $course_cats ) {
	$last_key = array_search( end( $course_cats ), $course_cats );
	foreach ( $course_cats as $key => $cat ) {
		$course_cats_string .= $cat->slug;
		if ( $key !== $last_key ) {
			$course_cats_string .= ',';
		}
	};
...
?>
	...
	<div class="card course-card" data-course="<?php echo esc_attr($course->ID);?>" id="course-card-<?php echo esc_attr($course->ID);?>" data-categories="<?php echo esc_attr($course_cats_string);?>">
		...
		<div class="card-content">
			...
			<a href="<?php echo $permalink;?>" class="is-size-5 has-text-weight-bold mb-4"><?php echo $course->post_title;?></a>
